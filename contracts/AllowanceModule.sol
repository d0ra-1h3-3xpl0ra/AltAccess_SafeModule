...
interface GnosisSafe {
    ...
    /// @dev WIP function to check wheter transfer signature is valid (for Safe with N Owners with threshold > 1)
...
contract AllowanceModule is SignatureDecoder {
    ...
    // Module can only transfer tokens fom this safe
    address public targetSafe;

    // Module can only transfer this token
    ...
    event AddTargetSafe(address indexed safe);
    event AddTargetToken(address indexed safe, address targetToken);
    ...
    function getAllowance(address safe, address delegate, address token) private view returns (Allowance memory allowance) {
        ...
        if (allowance.resetTimeMin > 0 && allowance.lastResetMin <= currentMin - allowance.resetTimeMin) {
            // set allowance.spent to equal amount, no more can be withdrawn
            allowance.spent = allowance.amount;
    ...
    function executeAllowanceTransfer(
        ...
        // Require the SAFE to be equal to TargetSafe, optional feature to lock module to a specific safe and token to be withdrawn
        require(targetSafe == address(safe), "Safe is not equal to TargetSafe");
        require(targetToken == token, "Safe is not equal to TargetToken");
    ...
    /// @dev Allows to add a delegate. Can be called multiple times with same delegate without error.
    ...
    /// @dev Specify the target token address to be used for allowance transfer
    /// @param _token Address of the token to be used for this module
    function setTargetToken(address _token) public {
        // require(_token != address(0), "Token address cannot be 0");
        targetToken = _token;
        emit AddTargetToken(msg.sender, targetToken);
    }

    /// @dev Returns the target token address
    /// @return targetToken Address of the token to be used for allowance transfer
    function getTargetToken() public view returns (address) {
        return targetToken;
    }

    /// @dev Specify the target safe address to be used for allowance transfer
    /// @param _safe Address of the safe to be used for this module
    function setTargetSafe(address _safe) public {
        targetSafe = _safe;
        emit AddTargetSafe(targetSafe);
    }

    /// @dev Return the target safe address, limits the module to be used only for the specified safe
    /// @return targetSafe Address of the safe to be used for this module
    function getTargetSafe() public view returns (address) {
        return targetSafe;
    }
}
