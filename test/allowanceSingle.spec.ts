...
// import { allowanceModuleSol } from '../typechain-types/contracts'
...
describe('AllowanceModule allowanceSingle', () => {
  ...
  it('Execute multiple ether allowance with delegate', async () => {
    ...
    const { safe, allowanceModule, owner, alice, token } = await loadFixture(setup)
    ...
    await execSafeTransaction(
      safe,
      await allowanceModule.setTargetToken.populateTransaction(ethers.ZeroAddress),
      owner
    )
    /*
